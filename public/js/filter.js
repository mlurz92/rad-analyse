...
class FilterManager {
    ...
    async showFilterMenu(column, th) {
        ...
        if (!this.filterValues[column]) {
            ...
            this.table.querySelectorAll(`tbody tr:not([style*="display: none"]) td:nth-child(${Array.from(th.parentNode.children).indexOf(th) + 1})`).forEach(td => {
    ...
    getValueCount(column, value) {
        let count = 0;
        const columnIndex = Array.from(this.table.querySelector('thead tr').children)
            .findIndex(th => th.dataset.column === column);
        
        this.table.querySelectorAll(`tbody tr:not([style*="display: none"]) td:nth-child(${columnIndex + 1})`).forEach(td => {
        ...
        rows.forEach(row => {
            ...
            for (const [column, selectedValues] of Object.entries(this.activeFilters)) {
                if (selectedValues.size > 0) {
                    const columnIndex = Array.from(row.parentNode.parentNode.querySelector('thead tr').cells)
